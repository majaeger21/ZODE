...
(define (parse-fundef [exp : Sexp]) : FundefC
  (match exp
    [(list 'def ': (? symbol? name) ': (? symbol? args) ... ': body)
     (define casted-args (cast args (Listof Symbol)))
     (cond
       [(valid-identifier? name) (error "ZODE: invalid name, got: ~e" name)]
       [(not (unique-args? casted-args)) (error "ZODE: invalid args, got: ~e" args)]
       [else (FundefC name casted-args (parse body))])]
    [other (error "ZODE: expected valid expression, got: ~e" other)]))
...
(define (interp [exp : ExprC] [funs : (Listof FundefC)]) : Real
  (match exp
    ...
    [(BinOpC s l r) (match s
                      ...
                      ['* (* (interp l funs) (interp r funs))]
                      ['- (- (interp l funs) (interp r funs))]
                      ['/ (let ([il (interp l funs)] [ir (interp r funs)])
                            (cond
                              [(equal? ir 0) (error 'interp "ZODE: Divide By Zero Error")]
                              [else (/ (interp l funs) (interp r funs))]))])]
    ...
    [(AppC s args) (let ([fd (getFunDef s funs)])
                     (cond
                       [(= (length args) (length (FundefC-args fd))) (interp (subst-all
                              (FundefC-body fd) args (FundefC-args fd) funs) funs)]
...
(check-exn #rx"ZODE: invalid name" (lambda () (parse-fundef '{def : + : f : x y : (+ x y)})))
(check-exn #rx"ZODE: expected valid expression" (lambda () (parse-fundef '{def + : f : x y : (+ x y)})))
(check-exn #rx"ZODE: expected valid expression" (lambda () (parse-fundef '{def : f : 1 2 : (+ X y)})))
(check-exn #rx"ZODE: expected valid expression, got: " (lambda () (parse-fundef '{})))
(check-equal? (parse-fundef '{def : f : x y : (+ x y)})
              (FundefC 'f (list 'x 'y)
                       (BinOpC '+ (IdC 'x) (IdC 'y))))
(check-exn #rx"ZODE: expected valid expression, got: " (lambda () (parse-fundef '{})))
(check-exn #rx"ZODE: invalid args" (lambda () (parse-fundef '{def : f : x x : (+ x x)})))
...
;subst test cases
(check-equal? (subst (NumC 3) 'x (BinOpC '+ (IdC 'x) (BinOpC '* (NumC 6) (IdC 'x)))) (BinOpC '+ (NumC 3)
                             (BinOpC '* (NumC 6) (NumC 3))))
(check-equal? (subst (NumC 3) 'x (BinOpC 'x (IdC 'x) (AppC 'f (list (IdC 'x) (IdC 'x))))) (BinOpC 'x
                        (NumC 3) (AppC 'f (list (NumC 3) (NumC 3)))))
(check-equal? (subst (NumC 3) 'x (BinOpC 'x (IdC 'y) (AppC 'f (list (IdC 'x) (IfLeqZeroC (IdC 'x) (NumC 5)
                   (NumC 6)))))) (BinOpC 'x (IdC 'y) (AppC 'f (list (NumC 3) (IfLeqZeroC (NumC 3) (NumC 5) (NumC 6))))))
...
;top-interp tests
(check-equal? (top-interp '{{def : f : x y : {ifleq0? : x : y : {+ y 1}}} {def : main : : {f 3 5}}}) 6)

(check-equal? (top-interp '{{def : f : x y : {+ x y}}
                     {def : main : : {f 1 2}}}) 3)

(check-exn #rx"ZODE: Function Application Error" (lambda () (top-interp '{{def : f : x y :
                       {ifleq0? : x : y : {+ y 1}}} {def : main : : {g 3 5}}})))

(check-equal? (top-interp '{{def : main : : {ifleq0? : 0 : 1 : 2}}})
              1)

(check-exn #rx"ZODE: Divide By Zero Error" (lambda () (top-interp '{{def : f : x y : {/ x y}}
                     {def : main : : {f 1 0}}})))
(check-exn #rx"ZODE: Divide By Zero Error" (lambda () (top-interp '((def : ignoreit : x :
                                        (+ 3 4)) (def : main : : (ignoreit (/ 1 (+ 0 0))))))))
